...
class ModbusPlatform {
  ...
  update() {
    ...
    // Give each characteristic its own command.
    for (let i = 0; i < this.charList.length; i++) {
      let char = this.charList[i];
      this.commands.push({
        cmd: "r",
        type: char.type,
        add: char.add,
        count: char.map.len || 1,
      });
    }
  ...
  onQueue() {
    ...
    if (this.command.cmd == "w") {
      ...
      const func = commandTypes[this.command.type];

      if (!this.modbus[func]) {
        this.command = null;
        Log(`Unknown read function ${func}`);
        return;
      }

      this.modbus[func](this.command.add - 1, Math.round(this.command.val));
      this.command = null;
      ...
      this.modbus[func](this.command.add - 1, this.command.count)
        .then((resp) => {
          const k = this.command.type.toString() + this.command.add.toString();
      ...
      // The execute ("x") command.
    } else if (this.command.cmd == "x") {
      this.charList.forEach((obj) => {
        const k = obj.type.toString() + obj.add.toString();
        const val = this.status[k];
      ...
      if (this.commands.length) this.socket.emit("modbus");
    }
  }
...
class ModbusAccessory {
    ...
    this.configs.forEach((config) => {
      ...
      for (let charType in config) {
        ...
        if (charType[0] < "A" || charType[0] > "Z") {
          Log(
            "unknown config element (remember characteristics are case sensitive)",
            charType
          );
          continue;
        }
        ...
        let characteristic = service.getCharacteristic(
        ...
        if (
          !this.platform.minModbus[modbusType] ||
          this.platform.minModbus[modbusType] > modbusAdd
        ) {
          this.platform.minModbus[modbusType] = modbusAdd;
        }
        if (
          !this.platform.maxModbus[modbusType] ||
  ...
  update(characteristic, val, map) {
    const now = Date.now();
    if (now < this.lastUpdate + 1000 && !this.platform.firstInit) {
      return;
    }
    if ("scale" in map) {
      val = val / map.scale;
    ...
    if (val != characteristic.value) {
      ...
      if (map.log) {
        if (!logFile)
          logFile = require("fs").createWriteStream("logfile.csv", {
            flags: "a",
          });
        if (logFile)
          logFile.write(
            new Date().toISOString() + "," + this.name + "," + val + "\n"
          );
      }
    }
  }
